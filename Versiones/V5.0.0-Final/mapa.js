  ...
  directionsService.route(request, function(response, status) {
    if (status === 'OK') {
      ...
      routeMarker = new google.maps.Marker({
        ...
        map: map,
        icon: {
          url: 'carro.png', // Ruta de la imagen del carro
          scaledSize: new google.maps.Size(32, 32) // Tamaño personalizado del carro (puedes ajustar según tus preferencias)
        }
      });
...
// Función para sumar los puntajes de la tabla
function sumTableScores() {
  var tableRows = document.querySelectorAll('#streetTable tbody tr');
  var tableTotal = 0;
  tableRows.forEach(function(row) {
    var score = parseInt(row.cells[1].textContent);
    tableTotal += score;
  });
  return tableTotal;
}
...
document.getElementById('quizForm').addEventListener('submit', function(event) {
  ...
  // Calcula el puntaje total combinado sumando los puntajes del cuestionario y los puntajes de la tabla
  const tableTotalScore = sumTableScores();
  const combinedTotalScore = totalScore + tableTotalScore;

  // Calcular el costo del seguro
  const insuranceQuote = quoteInsurance(combinedTotalScore);

  // Mostrar el puntaje total combinado y el costo del seguro al usuario
  document.getElementById('result').innerText = `Tu puntaje es: ${combinedTotalScore}\n${insuranceQuote}`;
});

// Función para calcular el costo del seguro en base al puntaje total
function quoteInsurance(combinedTotalScore) {
  let insuranceCost;

  if (combinedTotalScore >= 20 && combinedTotalScore <= 29) {
    insuranceCost = 4500;
  } else if (combinedTotalScore >= 30 && combinedTotalScore <= 39) {
    insuranceCost = 6000;
  } else if (combinedTotalScore >= 40 && combinedTotalScore <= 49) {
    insuranceCost = 7500;
  } else if (combinedTotalScore >= 50 && combinedTotalScore <= 59) {
    insuranceCost = 9000;
  } else if (combinedTotalScore >= 60) {
    insuranceCost = 11500;
  } else {
    insuranceCost = 0; // Caso por si el puntaje no cae en ninguno de los rangos
  }

  return `El costo de tu seguro es de: $${insuranceCost}`;
}
